...
def _build_token_response(user: dict) -> TokenResponse:
    ...
    return TokenResponse(
        ...
        user=UserResponse(
            ...
            created_at=user.get("created_at", user.get("updated_at", datetime.utcnow())),
        ),
    )
...
async def login_user(
    ...
    if not password_hash or not auth_service.verify_password(login_data.password, password_hash):
...
async def refresh_token(
    ...
    if refresh_data is not None:
        ...
        current_user = await db.users.find_one({"email": payload["sub"]})
        if current_user is None or not current_user.get("is_active", False):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Refresh token or valid authorization is required",
        )
...
async def update_profile(
    ...
    try:
        ...
        if "full_name" in update_data:
            ...
            if len(update_data["full_name"]) < 2:
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail="Full name must contain at least two characters",
                )
        for optional_text in ("phone", "location"):
            if optional_text in update_data and update_data[optional_text] is not None:
                update_data[optional_text] = update_data[optional_text].strip() or None
...
async def change_password(
    ...
    if not current_user.get("password_hash"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Password cannot be changed for this account")
    ...
    if not any(char.isupper() for char in password_data.new_password):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="New password needs an uppercase letter")
    if not any(char.islower() for char in password_data.new_password):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="New password needs a lowercase letter")
    if not any(char.isdigit() for char in password_data.new_password):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="New password needs a number")

    await db.users.update_one(
        {"_id": current_user["_id"]},
        {"$set": {"password_hash": auth_service.hash_password(password_data.new_password), "updated_at": datetime.utcnow()}},
    )
...
async def get_notification_preferences(current_user: dict = Depends(get_current_user)):
    """Return backend-enforced alert preferences for the current account."""
    return {
        "unsafe_water_alerts": current_user.get("alert_on_unsafe", True),
        "contamination_risk": current_user.get("alert_on_high_risk", True),
        "tank_level_alerts": current_user.get("alert_on_tank_critical", True),
        "push_enabled": current_user.get("push_enabled", True),
    }
...
async def update_notification_preferences(
    ...
    """Persist the alert switches used by in-app history and FCM delivery."""
    update = {
        "alert_on_unsafe": preferences.unsafe_water_alerts,
        "alert_on_high_risk": preferences.contamination_risk,
        "alert_on_tank_critical": preferences.tank_level_alerts,
...
async def update_fcm_token(
    ...
    """Associate the current phone's push token with the signed-in account."""
    token = token_data.token.strip() if token_data.token else None
    await db.users.update_one(
        {"_id": current_user["_id"]},
        {"$set": {"fcm_token": token, "updated_at": datetime.utcnow()}},
    )
    return {"status": "success", "registered": bool(token)}
