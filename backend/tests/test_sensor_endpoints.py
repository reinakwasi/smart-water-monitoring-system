...
# Requests still resolve the application's database dependency even when the
# endpoint-local database object is mocked.
pytestmark = pytest.mark.usefixtures("setup_test_db")



@pytest.fixture(autouse=True)
def authenticated_test_device():
    """Endpoint behavior tests run as a registered ESP32 device."""
    async def authenticate():
        return {"device_id": "ESP32_TEST_001", "device_name": "Test ESP32"}

    app.dependency_overrides[verify_device_api_key] = authenticate
    yield
    app.dependency_overrides.pop(verify_device_api_key, None)
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        with patch('app.api.v1.endpoints.sensor.ml_service') as mock_ml_service, \
            ...
            mock_shap_service.explain_classification.return_value = {
                ...
                'top_factors': [
                    {'feature': 'turbidity_index', 'shap_value': 0.45, 'direction': 'positive'},
            ...
            mock_shap_service.explain_risk_prediction.return_value = {
                ...
                'top_factors': [
                    {'feature': 'turbidity_index', 'shap_value': 0.18, 'direction': 'positive'},
