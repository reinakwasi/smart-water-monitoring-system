...
async def register_and_verify(async_client, user_data):
    """Register a test user and complete the real OTP verification flow."""
    response = await async_client.post("/api/v1/auth/register", json=user_data)
    assert response.status_code == status.HTTP_201_CREATED

    otp = email_service.otp_store[user_data["email"]]["otp"]
    verification = await async_client.post(
        "/api/v1/auth/verify-otp",
        params={"email": user_data["email"], "otp": otp},
    )
...
class TestAuthService:
    ...
    def test_create_refresh_token(self):
        """Refresh credentials are distinguishable from access credentials."""
        data = {"sub": "test@example.com", "user_id": "123", "role": "user"}
...
class TestAuthEndpoints:
    ...
    async def test_login_success(self, async_client, db):
        ...
        await register_and_verify(async_client, user_data)
...
class TestAuthDependencies:
    ...
    async def test_require_admin_with_admin_user(self, async_client, db):
        ...
        await register_and_verify(async_client, admin_data)
