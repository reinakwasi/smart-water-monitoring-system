...
class MLService:
    ...
    def __init__(
        ...
        classifier_version: str = "v2.0",
        risk_predictor_version: str = "v2.0"
    ):
        """
        ...
        # These are the four physical probes installed on the ESP32.
    ...
    def predict_contamination_risk(
        ...
        if historical_readings is not None and len(historical_readings) >= window_size:
            ...
            # Current + mean + standard deviation + trend for four sensors.
            feature_count = len(self.classification_features)
        ...
        if task_type == 'classification':
            # Convert class probabilities into a severity score. The maximum
            # probability is model confidence, not contamination risk.
            ...
            named_severity = {'low': 0.0, 'medium': 1.0, 'high': 2.0}
            severity_values = []
            for class_name in class_names:
                label = str(class_name).lower()
                try:
                    severity_values.append(float(class_name))
                except (TypeError, ValueError):
                    severity_values.append(named_severity.get(label, 0.0))

            max_severity = max(max(severity_values), 1.0)
            risk_score = float(sum(
                probability * (severity / max_severity)
                for probability, severity in zip(probabilities, severity_values)
            ))
    ...
    def _extract_temporal_features(
        ...
        - Mean of historical readings (4 features)
        - Std of historical readings (4 features)
        - Trend of historical readings (4 features)
