...
// ESP32 + TDS Sensor + Backend Integration
// Sends ONLY TDS data to FastAPI backend
...
bool sendDataToBackend(float tds_value) {
  ...
  // Create JSON payload
  // Backend expects: device_id, timestamp, ph, turbidity_index, temperature, tds, dissolved_oxygen
  ...
  doc["turbidity_index"] = 5.0;               // Default low turbidity (0-100 scale)
  doc["temperature"] = temperature;           // Using global temperature
  doc["tds"] = tds_value;                     // ACTUAL TDS VALUE
  doc["dissolved_oxygen"] = 8.5;              // Default good DO level
